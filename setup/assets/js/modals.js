...
const OwlModal = {
  ...
  prompt: function(title, message, defaultValue, onConfirm, confirmText = 'Submit', confirmColor = 'var(--primary)', cancelText = 'Cancel') {
    this.create();
    const box = document.getElementById('owl-modal-box');
    box.innerHTML = `
      <h3 style="margin-bottom: 0.5rem; font-size: 1.25rem;">${title}</h3>
      <p style="color: var(--text-dim); margin-bottom: 1.5rem; font-size: 0.95rem;">${message}</p>
      <input type="text" id="owl-modal-input" value="${defaultValue || ''}" style="width: 100%; margin-bottom: 1.5rem; padding: 0.75rem; background: rgba(255,255,255,0.05); border: 1px solid var(--border); border-radius: var(--radius-sm); color: white; outline: none; font-size: 1rem;">
      <div style="display: flex; gap: 1rem;">
        <button class="btn btn-outline" style="flex: 1;" onclick="OwlModal.close()">${cancelText}</button>
        <button class="btn btn-primary" id="owl-modal-confirm-btn" style="flex: 1; background: ${confirmColor};">${confirmText}</button>
      </div>
    ...
    setTimeout(() => {
        const input = document.getElementById('owl-modal-input');
        if (input) { input.focus(); input.select(); }
    }, 50);

    document.getElementById('owl-modal-confirm-btn').onclick = () => {
      const val = document.getElementById('owl-modal-input').value;
      onConfirm(val);
      this.close();
    };
  },

...
async function signOut() {
  ...
  window.location.href = isLocal ? 'auth/login.html?logout=true' : 'auth/login?logout=true';
}
...
async function signInWithProvider(strategy) {
  ...
  try {
    await clerk.client.signIn.authenticateWithRedirect({
      strategy: strategy,
      redirectUrl: currentOrigin + '/auth/sso-callback.html',
      redirectUrlComplete: currentOrigin + '/dashboard.html'
    });
  } catch (err) {
    if (err.errors?.[0]?.code === 'session_exists' || err.message?.includes('already signed in')) {
      window.location.href = currentOrigin + '/dashboard.html';
      return;
    }
    throw err;
  }
...
async function handleCallback() {
  ...
  await clerk.handleRedirectCallback({
    redirectUrlComplete: window.location.origin + '/dashboard.html'
  });
}
